...
async function handelPayment(req, res) {
  try {
    ...
    const check = await model.payment.findOne({ where: [{ cardNumber: cardNumber }, { cvv: cvv }, { expirationDate: expirationDate }] });
...
model.service.paymentFunction = async function (ID) {
  ...
  return { numberService: numberService, foundUser: foundUser };
};
